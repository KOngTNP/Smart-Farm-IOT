...
void connectBlynk() {
  ...
  while (Blynk.connect() == false) {
    connectCount++;
    delay(500);
    Serial.print("Connecting to local Blynk..");
    if (connectCount > 50) ESP.restart();
  }
  ...
  while (Blynk.connect() == false) {
    ConnectCount++;
    delay(500);
    Serial.print("Connecting to official Blynk..");
    if (ConnectCount > 50) ESP.restart();
  }
#endif
...
void blynkConnectTask(void *p) {
  while (1) {
    if (!Blynk.connected()) {
      if (Blynk.connect()) {
        BLYNK_LOG("Reconnected Task");
      } else {
        BLYNK_LOG("Not reconnected Task");
        forceStopAllValve();
      }
    }
...
void updateBlynk() {
  ...
  Blynk.virtualWrite(V2, moistureData*10);
