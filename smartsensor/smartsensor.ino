...
//it can use many devrice but in this case i use only one devices
char* FLORA_DEVICES[] = {
  "C4:7C:8D:6B:34:34"
};

//setting data from devices
...
void loop() {
  ...
  onReceive(LoRa.parsePacket());  //wait 5 min incoming command

  if (millis() > 300000) hibernate();
...
void readBattery() {
  ...
  batteryData = (batteryData * 17.9 / 4095);
...
void hibernate() {
  ...
  Serial.print("Sleep for "); Serial.print(sensorRoundSet); Serial.println(" min");
  esp_sleep_enable_timer_wakeup((sensorRoundSet * SLEEP_DURATION) * 1000000);
  delay(100);
...
void delayedHibernate(void *parameter) {
  ...
  Serial.println("Something went wrong, entering emergency hibernate mode");
  hibernate();
}
